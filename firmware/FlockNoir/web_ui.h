...
<header>
  <div class="brand-plate"><img src="/logo.png" alt="Flock Noir"
       onerror="this.parentNode.innerHTML='<span style=&quot;color:#04120b;font-weight:800;letter-spacing:2px&quot;>FN</span>'"></div>
  <div class="brand-txt">
...
<div class="tabs">
  ...
  <button class="tab" data-tab="pig" hidden>PIG DETECTOR</button>
...
<div class="wrap">
  <div class="card" id="modeCard" hidden>
    <div class="row-head"><strong id="activeMode">Loading mode...</strong><span style="flex:1"></span>
      <label class="radio"><input id="modeMute" type="checkbox"> mute alerts</label></div>
    <div class="hint" id="modeResources"></div>
    <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:10px">
      <button class="btn sm" id="dashboardCoverage">DASHBOARD ON</button>
      <button class="btn sm" id="fieldCoverage">FIELD SCAN · HOTSPOT OFF</button>
      <span class="hint" id="coverageState"></span></div>
    <div class="hint">Choosing a scan tab starts that mode. Camera and Settings leave it running. Hold BOOT for 1.5 seconds to restore the hotspot.</div>
  </div>
  <!-- ============ DETECTOR ============ -->
  <section id="detector">
    ...
    <div class="card" id="fusionCard">
      <div class="row-head"><strong>ALPR focus</strong><span style="flex:1"></span><button class="btn primary" id="alprProfile">START ALPR MODE</button></div>
      ...
      <div class="hint">ALPR mode runs OPT101, camera analysis and only ALPR / Flock radio rules. Field scan hops priority channels 1/6/11. Enable OPT101 below after wiring. The radio shares time between WiFi and BLE.</div>
    </div>
  ...
  <section id="camera" hidden>
    <div class="card">
      ...
      <div class="cam-wrap" id="camWrap">
        ...
        <div class="cam-mark"><img src="/logo.png" alt="Flock Noir"></div>
      </div>
  ...
  <section id="scanner" hidden>
    ...
    <div class="card">
      ...
      <a class="btn sm" href="/api/radio/log" download>DEVICE EVENTS</a>
    </div>
    <div class="card" id="radioCard" hidden>
      ...
      <div class="hint" id="radioModeHint">General mode scans WiFi / BLE signatures, watchlists and drone Remote ID. Optical analysis runs in ALPR mode.
        <b>Field mode turns off the hotspot</b> and hops the selected channels. Hold <b>BOOT for 1.5 seconds</b>
        to restore this dashboard. Radio coverage does not change the selected scan mode.</div>
      <div class="grid">
  ...
  <section id="pig" hidden>
    <div class="banner clear" id="pigBanner"><span id="pigBannerTxt">PIG DETECTOR · AXON BLE</span></div>
    <div class="card">
      <div class="row-head"><strong>Axon body camera watch</strong><span style="flex:1"></span><button class="btn primary" id="pigProfile">START PIG DETECTOR</button></div>
      <div class="hint">Continuously listens for Axon Bluetooth advertisements. WiFi detection, camera analysis, OPT101 sampling, wardriving and other device alerts stop in this mode.</div>
      <div class="hint" id="pigHealth"></div>
      <div class="hint">An Axon vendor match means a possible nearby Axon device; it does not prove a body camera or that it is recording. Silent devices cannot be detected. RSSI is signal strength, not distance.</div>
      <div class="grid">
        <div class="field"><label for="axonRepeat">Repeat siren while evidence is fresh (seconds)</label><input id="axonRepeat" type="number" min="5" max="60" value="10"></div>
        <div class="field"><button class="btn" id="axonSave">SAVE REMINDER</button><div class="hint">5–60 seconds. Stops after 3 seconds without a matching advertisement. Change the Axon sound in Settings.</div></div>
      </div>
      <div style="overflow-x:auto"><table><thead><tr><th>Device / name</th><th>Evidence</th><th>RSSI</th><th>Age</th></tr></thead><tbody id="pigRows"></tbody></table></div>
      <a class="btn sm" href="/api/radio/log" download>RADIO EVENT LOG</a>
    </div>
  </section>
  ...
  <section id="wardrive" hidden>
    <div class="card">
      <div class="row-head"><strong>WiFi + Bluetooth wardriver (WiGLE)</strong><span style="flex:1"></span>
        <label class="switch"><input type="checkbox" id="wdEn"><span class="slider"></span></label></div>
      <div class="hint" style="margin-top:0">Wardrive only: nearby 2.4 GHz WiFi networks and Bluetooth Low Energy advertisements go to <code>/wardrive/wigle_*.csv</code>. Detection rules, automatic sounds and optical analysis are paused. The ESP32-S3 supports BLE, not Bluetooth Classic.</div>
      <div class="hint">Use FIELD SCAN above for all-channel WiFi coverage (1–11); BLE continues. With a phone connected to the dashboard, WiFi reception stays on its current channel. Leaving this tab for another scan tab stops wardriving.</div>
      <div class="hint">Rows require a fresh GPS fix, UTC, altitude and HDOP. Accuracy is an HDOP × 5 m estimate. Repeat observations are saved every 15 seconds per address; device names with line breaks are normalized for WiGLE.</div>
      <div class="grid">
      ...
      <div class="hint" id="wdFixNote"></div><div class="hint" id="wdBreakdown"></div>
    </div>
  </section>
...
<footer>
  ...
  <span id="firmwareVersion">Flock Noir v0.6.0</span>
</footer>
...
/* ---- tabs ---- */
const SECTIONS=['detector','scanner','pig','camera','wardrive','settings'];
const MODE_NAMES={alpr:'ALPR',general:'GENERAL SCAN',axon:'PIG DETECTOR',wardrive:'WARDRIVE'};
const MODE_TABS={detector:'alpr',scanner:'general',pig:'axon',wardrive:'wardrive'};
let exclusiveModes=false,profileInitialized=false,profileBusy=false,activeProfile='';
function showTab(tab){
  $$('.tab').forEach(x=>x.classList.toggle('on',x.dataset.tab===tab));
  SECTIONS.forEach(s=>$('#'+s).hidden=tab!==s);
  if(tab!=='camera')camStop();
  if(tab==='camera')loadRecs();
  if(tab==='settings')loadSettings();
}

$$('.tab').forEach(b=>b.onclick=async()=>{
  const tab=b.dataset.tab,profile=MODE_TABS[tab];
  if(exclusiveModes && profile && profile!==activeProfile && !await setProfile(profile))return;
  showTab(tab);pollRadio();
});
...
$('#wdEn').onchange=async()=>{
  if(exclusiveModes){const enabled=$('#wdEn').checked;if(await setProfile(enabled?'wardrive':'general'))showTab(enabled?'wardrive':'scanner');return;}
  await fetch('/api/wardrive',{method:'POST',body:new URLSearchParams({en:$('#wdEn').checked?'1':'0'})});
};
const post=(u,en)=>fetch(u,{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:'en='+(en?'1':'0')});
$('#modeMute').onchange=()=>post('/api/mute',$('#modeMute').checked);
...
async function tick(){
  try{
    ...
    if(s.version)$('#firmwareVersion').textContent='Flock Noir v'+s.version;
    exclusiveModes=!!s.exclusiveModes;activeProfile=s.profile||'general';
    $('#modeCard').hidden=!exclusiveModes;$('[data-tab=pig]').hidden=!exclusiveModes;
    if(!profileInitialized && exclusiveModes){profileInitialized=true;showTab(Object.keys(MODE_TABS).find(t=>MODE_TABS[t]===activeProfile)||'detector');}
    $('#activeMode').textContent=(MODE_NAMES[activeProfile]||activeProfile)+' ACTIVE';
    $('#modeResources').textContent=({alpr:'OPT101 + camera + ALPR WiFi / BLE · ALPR CSV',general:'WiFi / BLE device signatures · device events · optics paused',axon:'Axon BLE only · repeating siren · optics and WiFi detection paused',wardrive:'WiFi + BLE survey · WiGLE CSV · detection alerts and optics paused'})[activeProfile]||'';
    if(document.activeElement!==$('#modeMute'))$('#modeMute').checked=!!s.muted;
    $('#recBtn').disabled=exclusiveModes && ['axon','wardrive'].includes(activeProfile);
    ...
    if(s.irDet){b.className='banner alert';
      ...
      $('#bannerTxt').textContent=(s.cameraAliased?'CAMERA ALIAS CANDIDATE  -  ':'CAMERA PULSE PATTERN  -  ')+fmt(s.freq,1)+' Hz, '+fmt(s.duty*100,0)+'% duty, conf '+fmt(s.confidence,2);}
    ...
    $('#readyIr').textContent=s.irPaused?'Paused by scan mode':!s.irEn?'Off — enable after wiring':s.irClipped?'Clipping — reduce light':fmt(s.irSampleHz,0)+' samples/s — verify light response';
    $('#readyCamera').textContent=exclusiveModes && !s.opticalActive?'Analysis paused by mode':s.cameraReady?fmt(s.fps,1)+' analysis fps':'Camera unavailable';
    const focused=s.profile==='alpr';
    $('#profileState').textContent=focused?'ALPR focus active — OPT101 + camera + ALPR radio detection':'ALPR mode inactive. Select this scan tab or START ALPR MODE to use all sensors.';
    $('#generalProfileState').textContent=activeProfile==='general'?'General alerts active — radio signatures only':'General mode inactive. Select Scanner to start general radio detection.';
    ...
    const axon=activeProfile==='axon' && ga && ga.category.startsWith('Axon') && ga.ageMs<=3000?ga:null;
    $('#pigBanner').className='banner '+(axon?'alert':'clear');
    $('#pigBannerTxt').textContent=axon?'POSSIBLE AXON BODY CAMERA NEARBY · '+axon.rssi+' dBm · '+axon.method:activeProfile==='axon'?'LISTENING FOR AXON BLE · NO RECENT MATCH':'PIG DETECTOR INACTIVE';
    ...
    $('#irStat').innerHTML = s.irPaused?'paused by mode':!s.irEn ? 'off'
    ...
    $('#count').textContent=exclusiveModes?(s.logged||0):(s.logged||0)+(s.radioEvents||0);
    ...
    $('#wdBreakdown').textContent='WiFi rows '+(s.wdWifiLogged||0)+' | BLE rows '+(s.wdBleLogged||0)+' | SD errors '+(s.wdErrors||0)+' | observations waiting for valid GPS '+(s.wdNoFix||0);
...
async function pollRadio(){
  ...
  try{
    ...
    $('#readyWifi').textContent=r.exclusiveModes && !r.wifiScanning?'Paused in Pig Detector':r.wifiReady?(r.mode==='field'?'Hopping '+(r.hop==='all'?'1–11':'1/6/11'):'Channel '+r.channel+' — field mode broadens coverage'):'Unavailable';
    $('#coverageState').textContent=r.mode==='field'?'Field coverage active':'Dashboard channel '+r.channel;
    $('#pigHealth').textContent=r.profile==='axon'?(r.bleScanning?'Scanning continuously':'BLE restarting / unavailable')+' | requested receive window '+r.bleWindowMs+'/'+r.bleIntervalMs+' ms'+(r.mode==='field'?' | WiFi off':' | shared with dashboard'):'Pig Detector is inactive';
    if(document.activeElement!==$('#axonRepeat'))$('#axonRepeat').value=r.axonReminderSeconds||10;
    $('#radioHealth').textContent=r.mode+' | channel '+r.channel;
    $('#radioCounts').textContent='WiFi frames '+(r.wifiFramesSeen??r.packets)+' | BLE ads '+(r.bleAdvertisementsSeen??'?')+' | queued '+r.packets+' | queue drops '+r.dropped+' | SD errors '+r.logErrors+
      (r.wifiReady?'':' | WiFi unavailable')+(r.ble && !r.bleReady?' | BLE unavailable':'')+
      (r.captureFull?' | Capture limit reached':'')+(r.detail?' | '+r.detail:'');
    $('#detectorRadioHealth').textContent='Flock-You scanning | '+(r.wifiFramesSeen??r.packets)+' WiFi frames / '+(r.bleAdvertisementsSeen??'?')+' BLE ads | '+r.mode+' | channel '+r.channel+
      ' | radio events '+(r.events||0)+(r.wifiReady?'':' | WiFi unavailable')+
      (r.ble && r.bleReady?' | BLE scanning':' | BLE off / unavailable');
    if(!$('#scanner').hidden || !$('#detector').hidden || !$('#pig').hidden){
      ...
      $('#pigRows').innerHTML=sorted.filter(d=>d.protocol==='BLE' && (d.category||'').startsWith('Axon')).map(render).join('')||'<tr><td colspan="4" class="dim">No Axon matches yet</td></tr>';
...
async function setProfile(profile){
  if(profileBusy)return false;profileBusy=true;
  try{
    const response=await fetch('/api/profile',{method:'POST',body:new URLSearchParams({profile})});
    if(!response.ok)throw new Error();
    activeProfile=profile;radioLoaded=false;
    await tick();await pollRadio();toast((MODE_NAMES[profile]||profile)+' active');return true;
  }catch(e){toast('Mode change failed — previous mode remains active');return false;}
  finally{profileBusy=false;}
}
$('#alprProfile').onclick=()=>setProfile('alpr');
$('#generalProfile').onclick=()=>setProfile('general');
$('#pigProfile').onclick=()=>setProfile('axon');
$('#axonSave').onclick=async()=>{
  try{
    const response=await fetch('/api/axon',{method:'POST',body:new URLSearchParams({repeatSeconds:$('#axonRepeat').value})});
    toast(response.ok?'Axon reminder saved':'Choose 5–60 seconds');
  }catch(e){toast('Could not save Axon reminder');}
};
async function coverage(mode){
  try{
    const response=await fetch('/api/coverage',{method:'POST',body:new URLSearchParams({mode})});
    if(!response.ok)throw new Error();
    toast(mode==='field'?'Hotspot off shortly — hold BOOT 1.5s to return':'Dashboard enabled');
  }catch(e){toast('Could not change radio coverage');}
}
$('#dashboardCoverage').onclick=()=>coverage('dashboard');
$('#fieldCoverage').onclick=()=>coverage('field');
